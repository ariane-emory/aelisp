...
int ae_fput(const ae_obj_t * const this, FILE * stream) {
  ...
  written    += fprintf(stream, "%s ", SHORT_TYPE_STR(GET_TYPE(this)));
...
static int ae_fwrite_internal(const ae_obj_t * const this) {
  ...
  case AE_ENV:
    if (NILP(ENV_PARENT(this)))
      COUNTED_FPRINTF(fwrite_stream, "%s<%018p→nil>", SHORT_TYPE_STR(GET_TYPE(this)), this);
    else
      COUNTED_FPRINTF(fwrite_stream, "%s<%018p→%018p>", SHORT_TYPE_STR(GET_TYPE(this)), this, ENV_PARENT(this));
    break;
  case AE_CORE:
    if (SPECIALP(this))
      COUNTED_FPRINTF(fwrite_stream,
                      "%s<%s, %018p, special>",
                      SHORT_TYPE_STR(GET_TYPE(this)),
                      CORE_NAME(this),
                      CORE_FUN(this));
    else
      COUNTED_FPRINTF(fwrite_stream,
